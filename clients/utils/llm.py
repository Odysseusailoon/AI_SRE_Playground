...
class OpenRouterClient:
    ...
    def inference(self, payload: list[dict[str, str]]) -> tuple[list[str], dict | None]:
        """
        Run inference and return response with usage metadata.
        
        Returns:
            (responses, usage_info)
            - responses: list[str] - LLM response content
            - usage_info: dict | None - Contains response_time, tokens, model info (None if from cache)
        """
        if self.cache is not None:
            cache_result = self.cache.get_from_cache(payload)
            if cache_result is not None:
                return cache_result, None  # No metadata for cached responses

        import time
        
        # Support multiple API sources
        api_source = os.getenv("API_SOURCE", "openrouter").lower()
        
        if api_source == "dashscope":
            # 阿里云 DashScope (Qwen models)
            client = OpenAI(
                api_key=os.getenv("API_KEY"),
                base_url=os.getenv("API_BASE", "https://dashscope.aliyuncs.com/compatible-mode/v1")
            )
            # Use MODEL from env if provided, otherwise use self.model
            model_to_use = os.getenv("MODEL", self.model)
        else:
            # OpenRouter (default)
            client = OpenAI(
        ...
        contents = [c.message.content for c in response.choices]  # type: ignore
        return contents, usage_info

    def run(self, payload: list[dict[str, str]]) -> tuple[list[str], dict | None]:
