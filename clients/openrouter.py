...
class OpenRouterAgent:
    ...
    async def get_action(self, input) -> str:
        ...
        try:
            ...
            response, usage_info = self.llm.run(trimmed_history)
            
            print(f"===== Agent (OpenRouter - {self.model}) ====\n{response[0]}")
            if usage_info:
                print(f"⏱️  响应时间: {usage_info['response_time']:.3f}s | "
                      f"Tokens: {usage_info['total_tokens']} "
                      f"(输入:{usage_info['prompt_tokens']}, 输出:{usage_info['completion_tokens']})")
            
            # Save to history with metadata
            history_item = {"role": "assistant", "content": response[0]}
            if usage_info:
                history_item["metadata"] = usage_info
