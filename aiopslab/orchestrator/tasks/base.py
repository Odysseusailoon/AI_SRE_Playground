...
class Task:
    ...
    def common_eval(self, trace: list[SessionItem]):
        ...
        # Original token estimation (for compatibility)
        self.add_result("in_tokens", in_tokens(trace))
        self.add_result("out_tokens", out_tokens(trace))
        
        # New: API-provided precise token and timing statistics
        api_stats = api_tokens_from_trace(trace)
        for key, value in api_stats.items():
            self.add_result(key, value)
        
        # Calculate environment execution time (TTA - LLM time)
        if api_stats.get('total_llm_time', 0) > 0:
