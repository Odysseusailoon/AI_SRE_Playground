...
class _TrainingJob:
    ...
    log_round: int = 0  # 轮次编号，用于日志目录命名（0, 1, 2...）
...
# 轮次计数器（用于日志目录命名）
# ================================
_JOB_ROUND_COUNTER = 0
_JOB_ROUND_LOCK = asyncio.Lock()


async def _get_cluster_pool() -> asyncio.Queue:
    """获取或初始化集群池"""
    global _CLUSTER_POOL
    async with _CLUSTER_POOL_LOCK:
        if _CLUSTER_POOL is None:
            _CLUSTER_POOL = asyncio.Queue()
            for cluster_name in _AVAILABLE_CLUSTERS:
                await _CLUSTER_POOL.put(cluster_name)
            logger.info(f"Initialized cluster pool with {len(_AVAILABLE_CLUSTERS)} clusters: {_AVAILABLE_CLUSTERS}")
        return _CLUSTER_POOL


async def _acquire_cluster() -> str:
    """从集群池获取一个可用集群（阻塞等待直到有集群可用）"""
    pool = await _get_cluster_pool()
    cluster_name = await pool.get()
    logger.info(f"Acquired cluster: {cluster_name}")
    return cluster_name
...
async def _release_cluster(cluster_name: str) -> None:
    """将集群归还到集群池"""
    pool = await _get_cluster_pool()
    await pool.put(cluster_name)
    logger.info(f"Released cluster: {cluster_name}")


async def _get_next_round_number() -> int:
    """获取下一个轮次编号（用于日志目录命名）"""
    global _JOB_ROUND_COUNTER
    async with _JOB_ROUND_LOCK:
        round_number = _JOB_ROUND_COUNTER
        _JOB_ROUND_COUNTER += 1
        return round_number
...
async def _run_single_episode(
    ...
    # 创建日志目录和文件（使用轮次编号）
    log_dir = FilePath("logs/echo") / str(job.log_round)
    log_dir.mkdir(parents=True, exist_ok=True)
    log_file = log_dir / f"task_{problem.problem_id}_{run_index}.log"
    
    try:
        # 设置环境变量，让 aiopslab 使用指定的 Kind 集群
        os.environ['AIOPSLAB_CLUSTER'] = cluster_name
        logger.info(f"▶ [Round {job.log_round}] Task {problem.problem_id}[{run_index}] started on cluster: {cluster_name}")
        
        # 创建日志文件并写入header
        with open(log_file, 'w') as f:
            f.write(f"=== Round: {job.log_round} ===\n")
            f.write(f"=== Task: {problem.problem_id} [{run_index}] ===\n")
...
async def start_training_job(payload: BatchRunRequest) -> JobStatusResponse:
    ...
    # 获取轮次编号（用于日志目录）
    log_round = await _get_next_round_number()
